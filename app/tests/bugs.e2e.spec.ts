...
import {DisplayBug} from "../src/lib/display-bug/displayBug";
...
describe('/bugs', () => {
    ...
    test('GET /bugs', (done) => {
        request(app)
            ...
            .end((err, res) => {
                ...
                const bugs: Array<DisplayBug> = res.body;
                expect(bugs).toHaveLength(sampleBugs.length);
                done();
            })
    })

    test('GET /bugs?assigneeId=2', (done) => {
        const assigneeId = 2;
        request(app)
            .get(`/bugs?assigneeId=${assigneeId}`)
            .expect(HttpStatus.OK)
            .end((err, res) => {
                if (err) {
                    logger.error(err.message);
                ...
                const sampleBugIds = sampleBugs
                    .filter((bug) => bug.assigneeId === assigneeId)
                    .map((bug) => bug.id);
    ...
    test('GET /bugs?query=1', (done) => {
        const query = '1';
        request(app)
            .get(`/bugs?query=${query}`)
            .expect(HttpStatus.OK)
            .end((err, res) => {
                if (err) {
                    logger.error(err.message);
    ...
    test('GET /bugs?query=cat&assigneeId=3', (done) => {
        const query = 'cat';
        const assigneeId = '3';
        request(app)
            .get(`/bugs?query=${query}&assigneeId=${assigneeId}`)
            .expect(HttpStatus.OK)
            .end((err, res) => {
                if (err) {
                    logger.error(err.message);
